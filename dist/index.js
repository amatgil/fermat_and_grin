...
export class Experiment {
    constructor(g, width, height) {
        ...
        let S1 = new Vec3(this.a / 2, 0, 0);
        let S2 = new Vec3(-this.a / 2, 0, 0);
        this.slits = [S1, S2];
        this.g = g;
        this.width = width;
        this.height = height;
    ...
    set_default_parameters() {
        ...
        let S1 = new Vec3(this.a / 2, 0, 0);
        let S2 = new Vec3(-this.a / 2, 0, 0);
        let S3 = new Vec3(0, this.a / 2, 0);
        let S4 = new Vec3(0, -this.a / 2, 0);
    ...
    recalculate_slits() {
        ...
        let S2 = new Vec3(-this.a / 2, 0, 0);
        this.slits = [S1, S2];
        let S3 = new Vec3(this.a / 2, this.a / 2, 0);
        let S4 = new Vec3(0, -this.a / 2, 0);
        this.slits = [S1, S2, S3, S4];
    }
    static to_world_space(i, j, width, height) {
        let x = (i / width) * REAL_WIDTH - REAL_WIDTH / 2;
        let y = (j / height) * REAL_HEIGHT - REAL_HEIGHT / 2;
        return [x, y];
    }
    electric_field(r, j, M) {
    ...
    draw_interference_pattern() {
        let pixels = new Uint8ClampedArray(4 * this.width * this.height);
        ...
        let image = new ImageData(pixels, this.width, this.height, { colorSpace: "srgb" });
        this.g.putImageData(image, 0, 0);
    }
}
Experiment.default_lambda = 5000; // Amstrongs
Experiment.default_a = 0.1; // mm
Experiment.default_L = 200; // mm
Experiment.default_T = 2 * PI; // radians 
Experiment.default_A = 300; // N/C
Experiment.default_iter = 10;
function start() {
    ...
    let experiment = new Experiment(g, width, height);
